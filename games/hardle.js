...
function renderControls(){
  ...
  bar.querySelector('#h-help').onclick = () => alert(
    "Hardle – How to play:\n" +
...
function renderBoard(){
  ...
  board.className = 'board board--with-counts'; // grid has a counts column
  for (let r=0; r<ROWS; r++){
    const row = document.createElement('div');
    row.className = 'row row--with-counts';
    row.dataset.row = r;
    ...
    counts.className = 'counts counts--cell'; // visible as a cell
    counts.id = `counts-${r}`;
...
function newGame(){
  ...
  host.querySelectorAll('.counts').forEach(c=>{ c.textContent='🟩 0  🟨 0'; c.removeAttribute('data-locked'); });
  kb?.reset();

  // choose a random answer that is NOT a plural ending in 's' (exceptions kept)
...
/* ---------- Coloring & keyboard sync ---------- */
function cycleTile(tile){
  if(gameOver) return;
  const r = +tile.dataset.row;
  const counts = host.querySelector(`#counts-${r}`);
  if(!counts || !counts.dataset.locked) return; // markable only after submit
  ...
  if(next==='gray'||next==='yellow'||next==='green') tile.classList.add(next);

  recomputeKeyboardFromSubmitted(); // gray now visible unless overridden by higher state
}
...
function recomputeKeyboardFromSubmitted(){
  ...
  for(let r=0;r<ROWS;r++){
    ...
    host.querySelectorAll(`.tile[data-row="${r}"]`).forEach(t=>{
      ...
      if(!/^[a-z]$/.test(ch) || !PRECEDENCE[st]) return;
      const cur = best[ch];
      if(!cur || PRECEDENCE[st] > PRECEDENCE[cur]) best[ch] = st;
    });
  }
...
async function animateWinRow(r){
  for(let i=0;i<COLS;i++){
    const t = tileAt(r,i);
    t.dataset.state='green';
    t.classList.remove('gray','yellow');
    t.classList.add('green','win-anim');
